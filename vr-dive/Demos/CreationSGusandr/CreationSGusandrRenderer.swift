...
// CreationSGusandrRenderer.swift
//
// Cube-container adaptation of ShaderToy "Creation S gusandr" (ssXXzn).
...
final class CreationSGusandrRenderer: VisualPatternController {
  let identifier: VisualPatternKind = .creationSGusandr
  ...
  init(device: MTLDevice, library: MTLLibrary, maxViewCount: Int) throws {
    ...
    let geo = CreationSGusandrRenderer.makeBox(
      device: device,
    ...
    pipelineState = try CreationSGusandrRenderer.makePipelineState(
      device: device,
      library: library,
      maxViewCount: self.maxViewCount)
    depthStencilState = CreationSGusandrRenderer.makeDepthStencilState(device: device)
  }
  ...
  func encodeFrame(encoder: MTLRenderCommandEncoder, context: PatternRenderContext) {
    ...
    var uniforms = CreationSGusandrUniforms(
      time: animationTime,
    ...
    encoder.setVertexBytes(
      &uniforms,
      length: MemoryLayout<CreationSGusandrUniforms>.stride,
      index: 1)
...
extension CreationSGusandrRenderer {
  ...
  fileprivate static func makePipelineState(
    ...
    desc.vertexFunction = library.makeFunction(name: "creationSGusandrVertex")
    desc.fragmentFunction = library.makeFunction(name: "creationSGusandrFragment")
