...
//
// ─── 设计方案 ────────────────────────────────────────────────────────────
// 思路: 用 2m×2m 网格 + 每格独立的 4 个边界墙 hash（东/北/西/南，西/南
//       通过复用相邻格子的东/北 hash 保证墙体不重复定义）近似生成随机
//       迷宫；每面墙用「厚度项 + 跨度容纳项 + 高度项」三者 max() 组合成
//       薄板 SDF。
// 关键参数:
//   - cellSize = 2.0、wallHalf = 0.04：格子大小与墙厚，决定通道净宽
//     ≈ cellSize - 2*wallHalf ≈ 1.9m（配合摄像机可通行的设计目标）。
//   - 每面墙出现概率 0.45：概率越高迷宫越封闭，越低越空旷。
// 性能特征: map() 最多检查 4 面墙 + 地板天花板，无迭代循环；march 120
//           步/maxD=25，开销中等。
// 已知限制/优化方向（⚠️ 曾踩坑记录，务必保留）:
//   - 「跨度容纳项」最初误写成 `边距 - cellSize*0.5`，导致该项恒 ≤ 0，
//     且在每面墙的跨度中心取值 ≈0（应为 ≈ -cellSize/2，即「远离边界，
//     不应生效」），从而在近 91% 的格子中央产生贯穿整格的幽灵墙，导致
//     地图看起来完全没有可通行的空腔。正确写法是 `-边距`（不减
//     cellSize*0.5，见下方 East/North/West/South 四处 `-dz`/`-dx`）。
//     若未来修改本文件的墙体公式，务必保持这个符号关系，否则会重现
//     同样的「假装可通行、实际处处堵死」的问题。
