...
fragment float4 dynamicBoxFragment(DynamicBoxVertexOut in [[stage_in]],constant DynamicBoxUniforms&u [[buffer(0)]],constant float4x4*v2w [[buffer(1)]],constant float4x4*vp [[buffer(2)]]) {
 ...
 ro=(u.patternTransform*float4(ro,1)).xyz;rd=normalize(float3(u.patternTransform*float4(rd,0)));float d0=0,t=u.time;
 for(int i=0;i<120;i++){float3 p=ro+rd*d0;float d=radialGillsDE(p,t);if(d<.0008f){float3 n=radialGillsN(p,t),l=normalize(float3(-.3,.9,.4));float dif=max(dot(n,l),0.f),rim=pow(1-max(dot(n,-rd),0.f),3.f),spec=pow(max(dot(reflect(-l,n),-rd),0.f),36.f);float3 c=mix(float3(.035,.19,.27),float3(.96,.72,.46),smoothstep(-.3,.4,p.z+p.x*.3));c=mix(c,float3(.83,.27,.31),smoothstep(.25,.52,p.y)*.45f);return float4(c*(.22+dif)+float3(.25,.6,.7)*rim*.3+spec*.5,1);}d0+=clamp(d*.68f,.001f,.05f);if(d0>6.0f)break;}
