...
uint vi=min(in.viewIndex,u.viewCount-1u);float3 ro=(v2w[vi][3].xyz-u.objectCenter.xyz)/u.boxScale,rd=normalize(in.worldPos-v2w[vi][3].xyz);float3 bn;if(!all(abs(ro)<DB_BOXDIMS-1e-3f)){float en=db_boxHit(ro,rd,DB_BOXDIMS,bn,true);if(en<0)return float4(.005,.01,.006,1);}ro=(u.patternTransform*float4(ro,1)).xyz;rd=normalize(float3(u.patternTransform*float4(rd,0)));float z, foldFar;
