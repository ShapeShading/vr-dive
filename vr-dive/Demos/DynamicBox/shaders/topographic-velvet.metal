...
uint vi=min(in.viewIndex,u.viewCount-1u);float3 ro=(v2w[vi][3].xyz-u.objectCenter.xyz)/u.boxScale,rd=normalize(in.worldPos-v2w[vi][3].xyz);float3 bn;if(!all(abs(ro)<DB_BOXDIMS-1e-3f)){float en=db_boxHit(ro,rd,DB_BOXDIMS,bn,true);if(en<0)return float4(.005,.01,.006,1);ro+=rd*(en+.002f);}ro=(u.patternTransform*float4(ro,1)).xyz;rd=normalize(float3(u.patternTransform*float4(rd,0)));float z, foldFar;
 if (!foldInterval(ro,rd,z,foldFar)) return float4(.004f,.007f,.012f,1.0f);
 float t=u.time;
for(int i=0;i<125;i++){float3 p=ro+rd*z;float d=topoDE(p,t);if(d<.0007f){float3 n=topoN(p,t),l=normalize(float3(-.45,.85,.4));float dif=max(dot(n,l),0.f),r=pow(1-max(dot(n,-rd),0.f),2.5f),sp=pow(max(dot(reflect(-l,n),-rd),0.f),48.f);float3 c=mix(float3(.025,.17,.14),float3(.80,.84,.48),smoothstep(-.35,.48,p.z));c=mix(c,float3(.13,.36,.58),smoothstep(.25,.52,p.y)*.3);return float4(c*(.22+dif)+float3(.2,.6,.3)*r*.26f+sp*.5f,1);}z+=clamp(d*.62f,.0009f,.045f);if(z>foldFar)break;}return float4(.003,.007,.005,1);}
