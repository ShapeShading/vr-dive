...
vertex Julia3DVertexOut julia3DVertexShader(
    ushort amplificationID [[amplification_id]],
    const device MeshVertex *vertices [[buffer(0)]],
    const device Julia3DParticleState *states [[buffer(1)]],
  ...
  Julia3DParticleState state = states[instanceID];
...
fragment float4 julia3DFragmentShader(Julia3DVertexOut in [[stage_in]],
...
// 暂时使用 Lorenz 方程，后续替换为 Julia 3D
inline float3 julia3DDerivative(float3 p, float sigma, float beta, float rho) {
  float3 d;
  d.x = sigma * (p.y - p.x);
  d.y = p.x * (rho - p.z) - p.y;
...
kernel void integrateJulia3DAttractor(device Julia3DParticleState *states
                                      [[buffer(0)]],
                                      constant Julia3DUniforms &uniforms
                                      [[buffer(1)]],
  ...
  Julia3DParticleState state = states[id];
  ...
  float3 k1 =
      julia3DDerivative(position, uniforms.sigma, uniforms.beta, uniforms.rho);
  float3 k2 = julia3DDerivative(position + 0.5f * dt * k1, uniforms.sigma,
                                uniforms.beta, uniforms.rho);
  float3 k3 = julia3DDerivative(position + 0.5f * dt * k2, uniforms.sigma,
                                uniforms.beta, uniforms.rho);
  float3 k4 = julia3DDerivative(position + dt * k3, uniforms.sigma,
                                uniforms.beta, uniforms.rho);
