...
// OctagramFieldCubeRenderer.swift
//
// Original implementation for a cube-portal raymarched scene of repeating
// animated box clusters.
...
final class OctagramFieldCubeRenderer: VisualPatternController {
  let identifier: VisualPatternKind = .octagramFieldCube
  ...
  private let travelSpeed: Float = 0.85
  ...
  init(device: MTLDevice, library: MTLLibrary, maxViewCount: Int) throws {
    ...
    let geo = OctagramFieldCubeRenderer.makeBox(device: device)
    vertexBuffer = geo.vertexBuffer
    indexBuffer = geo.indexBuffer
    indexCount = geo.indexCount

    pipelineState = try OctagramFieldCubeRenderer.makePipelineState(
      device: device, library: library, maxViewCount: self.maxViewCount)
    depthStencilState = OctagramFieldCubeRenderer.makeDepthStencilState(device: device)
  }
  ...
  func encodeFrame(encoder: MTLRenderCommandEncoder, context: PatternRenderContext) {
    ...
    var uniforms = OctagramFieldCubeUniforms(
      time: animationTime,
    ...
    encoder.setVertexBytes(
      &uniforms, length: MemoryLayout<OctagramFieldCubeUniforms>.stride, index: 1)
    ...
    encoder.setFragmentBytes(
      &uniforms, length: MemoryLayout<OctagramFieldCubeUniforms>.stride, index: 0)
...
extension OctagramFieldCubeRenderer {
  ...
  fileprivate static func makePipelineState(
    ...
    desc.vertexFunction = library.makeFunction(name: "octagramFieldCubeVertex")
    desc.fragmentFunction = library.makeFunction(name: "octagramFieldCubeFragment")
