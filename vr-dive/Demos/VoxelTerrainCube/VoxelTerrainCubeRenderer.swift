...
// VoxelTerrainCubeRenderer.swift
//
// Original implementation for a voxel terrain scene rendered through a cube portal.
...
// only follows the high-level idea of a lit voxel landscape seen through a cube.

final class VoxelTerrainCubeRenderer: VisualPatternController {
  let identifier: VisualPatternKind = .voxelTerrainCube
  ...
  init(device: MTLDevice, library: MTLLibrary, maxViewCount: Int) throws {
    ...
    let geo = VoxelTerrainCubeRenderer.makeBox(device: device)
    vertexBuffer = geo.vertexBuffer
    indexBuffer = geo.indexBuffer
    indexCount = geo.indexCount

    pipelineState = try VoxelTerrainCubeRenderer.makePipelineState(
      device: device, library: library, maxViewCount: self.maxViewCount)
    depthStencilState = VoxelTerrainCubeRenderer.makeDepthStencilState(device: device)
  }
  ...
  func encodeFrame(encoder: MTLRenderCommandEncoder, context: PatternRenderContext) {
    ...
    var uniforms = VoxelTerrainCubeUniforms(
      time: animationTime,
    ...
    encoder.setVertexBytes(
      &uniforms, length: MemoryLayout<VoxelTerrainCubeUniforms>.stride, index: 1)
    ...
    encoder.setFragmentBytes(
      &uniforms, length: MemoryLayout<VoxelTerrainCubeUniforms>.stride, index: 0)
...
extension VoxelTerrainCubeRenderer {
  ...
  fileprivate static func makePipelineState(
    ...
    desc.vertexFunction = library.makeFunction(name: "voxelTerrainCubeVertex")
    desc.fragmentFunction = library.makeFunction(name: "voxelTerrainCubeFragment")
