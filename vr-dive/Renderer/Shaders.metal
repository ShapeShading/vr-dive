...
kernel void simulateObjects(device ObjectState *states [[buffer(0)]],
  ...
  float scaledDt = dt * 0.45;
  float type = state.positionAndType.w;

  float3 velocity = state.velocityAndPhase.xyz;
  float phase = state.velocityAndPhase.w + scaledDt * (0.5 + 0.5 * type);

  float3 offset =
      float3(sin(phase * 1.3), cos(phase * 0.8), sin(phase * 1.1)) * 0.02;
  float buoyancy = mix(0.2, 0.35, type);
  velocity.y += buoyancy * scaledDt;
  velocity.x += sin(uniforms.globalTime * 0.2 + state.positionAndType.z) *
                0.01 * scaledDt;

  float3 position = state.positionAndType.xyz + velocity * scaledDt + offset;
